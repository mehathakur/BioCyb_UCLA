...
function thyrosim(dq, q, p, t)
    ...
    #TODO: Plug in ODEs for D1 dq[14] and D2 dq[15] and renumber delay ODEs
    # dq[14] = ... #D1
    # dq[15] = ... #D2
...
function initialize_immune()
    ...
    p[15] = 1 # NOTE: NEED TO FIT and CHANGE
...
"""
 System of ODEs defining the model.
"""
function immune_odes()
    # Immune ODEs
    dq[1] = p[1]*(q[4]/(q[4]+p[13]))*q[3]+p[15]*q[7]-(p[7]+p[2])*q[1] # Bdot
    dq[2] = p[2]*q[1]-p[8]*q[2] # Pdot
    dq[3] = p[3]*q[5]+p[16]*(q[4]/(q[4]+p[14]))*q[3]-p[9]*q[3] # Tdot
    dq[4] = p[4]*q[3]+p[10]*q[4] # Cdot
    dq[5] = p[5]*((q[8]/q[5])-p[17])-p[11]*q[5]*q[6]
    dq[6] = p[6]*q[2]-q[6]*(p[12]+p[11]*q[5])
    dq[7] = q[5]/p[18]*T4free
    dq[8] = q[5]/p[18]*T3free
